...
// Fetch month from the form input
$month = isset($_POST['month']) && !empty($_POST['month']) ? $_POST['month'] : '';

// Fetch all records based on the selected month
$query = "SELECT * FROM receipts WHERE month='$month' AND status=1 ORDER BY rno";
    ...
    <div class="content-page">
        <div class="content">
            <div class="container-fluid">
                <h1 class="page-title">Receipt Report by Month</h1>
                ...
                <form method="post" action="<?php echo $_SERVER['PHP_SELF']; ?>" enctype="multipart/form-data" class="noprint">
                    ...
                    <select name="month" id="month" required>
                        ...
                        // Assume $selectedMonth holds the current selected value
    $selectedMonth = isset($_POST['month']) ? $_POST['month'] : ''; // Example: Get the value from form submission

    foreach ($monthNames as $key => $month) {
        echo "<option value='$key' " . ($key == $selectedMonth ? "selected" : "") . ">$month</option>";
    }
    ?>
</select>
