...
// Fetch receipt number from the form input
$frrno = isset($_POST['frrno']) && !empty($_POST['frrno']) ? $_POST['frrno'] : '';
$torno = isset($_POST['torno']) && !empty($_POST['torno']) ? $_POST['torno'] : '';

// Fetch all records based on the selected receipt number
$query = "SELECT * FROM receipts WHERE rno BETWEEN '$frrno' AND '$torno' AND status=1 ORDER BY rno";
    ...
    <div class="content-page">
        <div class="content">
            <div class="container-fluid">
                <h1 class="page-title">Receipt Report by From  -  To </h1>
                <!-- Form for selecting receipt number -->
                <form method="post" action="<?php echo $_SERVER['PHP_SELF']; ?>" enctype="multipart/form-data" class="noprint">
                    <!-- Input field for selecting receipt number -->
                    <label for="rno">Receipt Number From:</label>
                    <input type="text" name="frrno" id="frrno" required>

                    <!-- Input field for selecting receipt number -->
                    <label for="rno">Receipt Number To:</label>
                    <input type="text" name="torno" id="torno" required>
                    ...
                    <tbody>
                        ...
                        foreach ($receipts as $receipt): ?>
                            <tr>
                                ...
                                <td><?= $receipt['month'] ?></td> <!-- month stored as name -->
                                <td><?= $receipt['year'] ?></td>
