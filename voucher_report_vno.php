...
$vno = isset($_POST['vno']) && is_numeric($_POST['vno']) ? intval($_POST['vno']) : '';

// Debugging - Check if the voucher number is set
echo "Voucher Number: " . $vno . "<br>";

if ($vno) {
    // Fetch all records for the given voucher number
    $query = "SELECT * FROM voucher WHERE vno='$vno' AND status=1 ORDER BY vno";
    ...
    <div class="content">
        <div class="container-fluid">
            ...
            <form method="post" action="<?php echo $_SERVER['PHP_SELF']; ?>" enctype="multipart/form-data" class="noprint">
                <!-- Only Voucher Number Input -->
                <label for="vno">Voucher Number:
                <input type="number" name="vno" id="vno" required></label>
